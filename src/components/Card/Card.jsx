...
function Card ({ Category, title, price, images }) {
  const { addedProducts, setAddedProducts, productsAdd, setProductsAdd } =
  ...
  return (
    <article className='relative border border-black cursor-pointer p-2 '>
      ...
      <button
        ...
        onClick={() => {
          setAddedProducts(addedProducts + 1); setProductsAdd([...productsAdd, {
            images,
            Category,
            title,
            price
      ...
      <div className='flex justify-around  text-sm absolute bottom-0  gap-10 bg-white py-2 w-width-price'>
